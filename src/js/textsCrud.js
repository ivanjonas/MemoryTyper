...
app.textsCrud = {
    ...
    editText: function editText(textId, newTitle, newText) {
        var textObj = app.textsCrud.getByTextId(textId),
            distance = Levenshtein.get(textObj.text.toLowerCase(), newText.toLowerCase());
    ...
    deleteText: function deleteText(textId) {
        app.textsCrud.texts = app.textsCrud.texts.filter(function(el, idx, arr) {
            return el.id !== textId;
        });
        app.textsCrud.persistTexts();
    ...
    persistTexts: function persistTexts() {
        localStorage.setItem("texts", JSON.stringify(app.textsCrud.texts));
    },
