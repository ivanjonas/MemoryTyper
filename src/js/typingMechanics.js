...
app.typingMechanics1 = {
    ...
    keyboardInput: function(event) {
        var text,
            target,
            tm = app.typingMechanics1,
            input = String.fromCharCode(event.keyCode),
            currentOutput = tm.$output.val(),
            currentOutputLength = currentOutput.length,
            initialOutputLength = currentOutputLength,
            options = app.settings.typingOptions;
        ...
        if (isInputCorrect()) {
            tm.$output.addClass("correct").removeClass("wrong"); /* TODO: run these feedback updates in a separate
        ...
        function printNextTargets() {
            var autoPrintPunctuation = !options['require-punctuation'],
                autoPrintNewline = !options['require-newline'];
            do {
